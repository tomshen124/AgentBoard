...
// Channels / SSH removed — stubs return empty results
async function executePluginAction(_params: any): Promise<{ success: boolean; error?: string }> {
  return { success: false, error: 'Channel plugins have been removed' }
}
...
import {
  ...
  ensureTaskLoopRunning
} from '../ipc/taskloop-policy-enforcer'
import {
  ...
  getBudgetInfo,
  estimateTokens
} from '../ipc/taskloop-context-injector'
...
const promptCacheKeyPrefix = 'agentboard'
...
function extractToolSummary(output: ToolResultContent): string {
  if (typeof output === 'string' && output.length > 0) {
    try {
      ...
      if (parsed && typeof parsed === 'object' && !Array.isArray(parsed)) {
        ...
        if (typeof r.stdout === 'string' && r.stdout.length > 0) return r.stdout.slice(0, 200)
...
function getPersistedProvidersState(): {
  ...
  return (
    decodePersistedStoreState<{
      ...
      activeFastProviderId?: string | null
      activeFastModelId?: string
    }>(root['agentboard-providers']) ?? { providers: [] }
  )
}

function getPersistedSettingsState(): Record<string, unknown> {
  const root = readSettings()
  return decodePersistedStoreState<Record<string, unknown>>(root['agentboard-settings']) ?? {}
}
...
async function* runAgentLoop(
  ...
  // Ensure TaskLoop sidecar is running for policy enforcement
  if (toolCtx.workingFolder) {
    void ensureTaskLoopRunning(toolCtx.workingFolder)
  }
  ...
  while (!hasIterationLimit || iteration < config.maxIterations) {
    ...
    for (const toolCall of toolCalls) {
      // ── TaskLoop hard block: policy engine explicitly denied this tool ──
      if (isToolHardBlocked()) {
        const reason = takeToolPolicyReason()
        const blockedAt = Date.now()
        const blockedOutput = encodeToolError(`Policy denied: ${reason}`)
        yield {
      ...
      // Record significant tool output to TaskLoop memory
      const outputPreview =
        typeof output === 'string' ? output.slice(0, 200) : extractToolSummary(output)
      if (outputPreview && !resultError) {
        const tags = [toolCall.name.toLowerCase()]
        if (toolCtx.workingFolder) tags.push('project')
        void recordTurnMemory('tool', `${toolCall.name}: ${outputPreview}`, tags)
      }
    }
    ...
    trackConversationTokens(
      estimateTokens(String(conversationMessages.map((m) => JSON.stringify(m)).join('')))
    )

    const budget = getBudgetInfo()
    if (budget.shouldCompact) {
      console.warn(
        `[TaskLoop] Context budget: ${budget.used}/${budget.limit} (${budget.percent}%) — compaction recommended`
      )
    }
...
async function resolveToolRequiresApproval(
  ...
  // ── TaskLoop policy check for Bash / Write ──
  if (name === 'Bash') {
    const command = String(input.command ?? input.cmd ?? '')
    if (command) {
      try {
        const check = await checkExecPolicy(command, ctx.workingFolder)
        lastToolPolicyCheck = check
        if (check.hardBlocked) return true
        if (check.requiresApproval) return true
        if (!check.allowed) return true
      } catch {
        lastToolPolicyCheck = null
      }
    }
  }
  if (name === 'Write') {
    ...
    if (filePath) {
      try {
        const check = await checkFileWritePolicy(filePath)
        lastToolPolicyCheck = check
        if (check.hardBlocked || check.requiresApproval || !check.allowed) return true
      } catch {
        lastToolPolicyCheck = null
      }
    }
...
function isToolHardBlocked(): boolean {
  return lastToolPolicyCheck?.hardBlocked === true
}

function takeToolPolicyReason(): string {
  const reason = lastToolPolicyCheck?.reason ?? ''
  lastToolPolicyCheck = null
  return reason
}
...
async function runCronAgentInternal(
  ...
  // ── Enrich system prompt with TaskLoop context and memory ──
  const enriched = await buildEnrichedSystemPrompt(definition.systemPrompt, jobId)
  ...
  initTokenBudget(enriched.tokenEstimate, innerProvider.maxTokens ?? 128_000)
