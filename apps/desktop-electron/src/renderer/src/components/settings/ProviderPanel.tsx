...
function getProviderProtocolLabel(t: (key: string) => string, type: ProviderType): string {
  if (type === 'anthropic') return t('provider.anthropicMessages')
  if (type === 'openai-responses') return t('provider.openaiResponses')
  if (type === 'gemini') return 'Gemini'
  return t('provider.openaiChatCompat')
}
...
function ProviderConfigPanel({ provider }: { provider: AIProvider }): React.JSX.Element {
  ...
  const homepageUrl = builtinPreset?.homepage
  const docsUrl = builtinPreset?.docsUrl
  const canOpenApiKeyUrl = isApiKeyAuth && provider.requiresApiKey !== false && !!apiKeyUrl
  const providerProtocolLabel = getProviderProtocolLabel(t, provider.type)
    ...
    <div className="flex flex-col h-full overflow-hidden">
      ...
      <div className="flex items-center justify-between px-5 py-4 border-b shrink-0">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-[11px] text-muted-foreground">{providerProtocolLabel}</p>
          </div>
        </div>
        <div className="flex items-center gap-2">
          {!provider.builtinId && (
      ...
      <div className="flex flex-1 min-h-0 flex-col overflow-y-auto overflow-x-hidden px-5 pt-4 pb-20">
          ...
          <section className="mb-4 flex flex-wrap items-center gap-2">
            {homepageUrl && (
              <Button
                ...
                onClick={() => void openExternal(homepageUrl)}
              >
                <ExternalLink className="size-3.5" />
                {t('provider.officialSite')}
              </Button>
            )}
            {docsUrl && (
              <Button
        ...
        <section className="space-y-2 mt-5">
          ...
          {provider.builtinId ? (
            <>
              <div className="inline-flex h-8 w-fit items-center rounded-md border border-border bg-muted/40 px-2.5 text-xs text-foreground">
                {t('provider.protocolAuto', { protocol: providerProtocolLabel })}
              </div>
              <p className="text-[11px] text-muted-foreground">{t('provider.protocolAutoHint')}</p>
            </>
          ) : (
            <>
              <Select
