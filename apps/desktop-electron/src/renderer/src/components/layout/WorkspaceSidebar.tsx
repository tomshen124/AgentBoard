...
const SIDEBAR_TREE_ROW_CLASS = 'workspace-sidebar-row min-h-8 rounded-lg border border-transparent'
...
const SIDEBAR_TREE_ACTION_BUTTON_CLASS = 'workspace-sidebar-row-action size-6 rounded-lg'
...
export function WorkspaceSidebar(): React.JSX.Element {
  ...
  const studioPageOpen = useUIStore((state) => state.studioPageOpen)
  const connectionsPageOpen = useUIStore((state) => state.connectionsPageOpen)
  ...
  const chatSurfaceActive =
    ...
    !studioPageOpen &&
    !connectionsPageOpen
  const featureMenuActive = studioPageOpen || resourcesPageOpen || skillsPageOpen || drawPageOpen
  ...
  const navigateProjectView = useCallback(
    (projectId: string, view: string = 'project') => {
  ...
  const navItems = [
    {
      key: 'new-chat',
      label: t('sidebar.newChat', { defaultValue: 'New Thread' }),
    ...
    {
      key: 'connectors',
      label: t('sidebar.connectorsLabel', { defaultValue: 'Connectors' }),
      icon: <Wand2 className="size-4 shrink-0" />,
      active: connectionsPageOpen,
      onClick: () => useUIStore.getState().openConnectionsPage()
    },
    {
      key: 'taskloop',
      label: t('sidebar.taskLoopLabel', { defaultValue: 'Automations' }),
      ...
      >
        ...
        >
          ...
          <div className="flex min-w-0 flex-1 items-center gap-2">
            <span className="agentboard-brand-glyph" aria-hidden="true">
              <span className="agentboard-brand-glyph-dot" />
            </span>
            <div className="min-w-0 truncate text-sm font-semibold text-sidebar-foreground/90">
              AgentBoard
            </div>
          </div>
        </div>
        ...
        <div className="flex min-h-0 flex-1 flex-col">
          <div className="space-y-1 px-2 py-1.5">
            ...
            <div className="group/resources-menu space-y-1">
              <button
                type="button"
                onClick={() => useUIStore.getState().openStudioPage()}
                className={cn(
                  'flex h-8 w-full items-center gap-2 px-2 text-[13px] font-medium transition-colors',
                  SIDEBAR_TREE_ROW_CLASS,
              ...
              >
                ...
                <span className="truncate">{t('sidebar.workbenchLabel')}</span>
                <ChevronRight
                  className={cn(
                ...
                <div className="space-y-1 overflow-hidden">
                  <button
                    ...
                    className={cn(
                      ...
                      SIDEBAR_TREE_ROW_CLASS,
                      studioPageOpen ? SIDEBAR_TREE_ACTIVE_CLASS : SIDEBAR_TREE_SUBITEM_HOVER_CLASS
                    )}
                  >
                    <FolderOpen className="size-3.5 shrink-0" />
                    <span className="truncate">{t('sidebar.assetsLabel')}</span>
                  </button>
                  <button
                    type="button"
                  ...
                  >
                    ...
                    <span className="truncate">{t('sidebar.canvasLabel')}</span>
                  </button>
                  <button
                    type="button"
                  ...
                  >
                    ...
                    <span className="truncate">{t('sidebar.skillsLabel')}</span>
                  </button>
                </div>
              </div>
            </div>
        ...
        <div className="mt-auto px-2 pb-2 pt-1.5">
          <div className="flex items-center gap-2">
            ...
            <Button
              ...
              onClick={() => useUIStore.getState().openSettingsPage('models')}
            >
