        ...
        <div className="flex h-screen flex-col items-center justify-center gap-6 p-8 text-center bg-background">
          ...
          <div className="space-y-2">
            <h2 className="text-lg font-semibold text-foreground">
              {i18n.t('errorBoundary.title', { ns: 'taskloop' })}
            </h2>
            <p className="max-w-md text-sm text-muted-foreground">
              {this.state.error?.message ||
                i18n.t('errorBoundary.defaultMessage', { ns: 'taskloop' })}
            </p>
          </div>
          ...
          <div className="flex items-center gap-2">
            ...
            >
              {i18n.t('errorBoundary.tryAgain', { ns: 'taskloop' })}
            </button>
            <button
            ...
            >
              {i18n.t('errorBoundary.reloadApp', { ns: 'taskloop' })}
            </button>
            <button
              className="rounded-md border px-4 py-2 text-sm font-medium text-muted-foreground hover:text-foreground hover:bg-muted transition-colors"
              onClick={this.handleCopyError}
            >
              {i18n.t('errorBoundary.copyError', { ns: 'taskloop' })}
            </button>
          </div>
          ...
          {this.state.error?.stack && (
            <details className="w-full max-w-lg text-left">
              <summary className="cursor-pointer text-xs text-muted-foreground hover:text-foreground transition-colors">
                {i18n.t('errorBoundary.errorDetails', { ns: 'taskloop' })}
              </summary>
