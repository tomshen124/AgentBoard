...
export interface LayeredMemorySnapshot {
  ...
  projectTools?: MemoryLayerEntry
  globalMemory?: MemoryLayerEntry
  projectMemory?: MemoryLayerEntry
  globalProfile?: MemoryLayerEntry
  globalFocus?: MemoryLayerEntry
  projectProfile?: MemoryLayerEntry
  projectFocus?: MemoryLayerEntry
...
export async function resolveGlobalMemoryHomePath(ipc: IPCClient): Promise<string | undefined> {
  ...
  try {
    ...
    cachedGlobalHomePath = joinFsPath(homeDirResult, '.agentboard')
    return cachedGlobalHomePath
  } catch {
    return undefined
  }
...
async function buildLayeredMemorySnapshot(
  ...
  const globalProfilePath = globalHomePath ? joinFsPath(globalHomePath, 'PROFILE.md') : undefined
  const globalFocusPath = globalHomePath ? joinFsPath(globalHomePath, 'FOCUS.md') : undefined
  ...
  const [
    ...
    scope === 'main' && projectRootPath
      ...
      : Promise.resolve(undefined),
    scope === 'main' && globalProfilePath
      ? loadOptionalMemoryFile(ipc, globalProfilePath)
      : Promise.resolve(undefined),
    scope === 'main' && globalFocusPath
      ? loadOptionalMemoryFile(ipc, globalFocusPath)
      : Promise.resolve(undefined),
    scope === 'main' && projectRootPath
      ? resolveProjectMemoryTextFileForTarget(
          ipc,
  ...
  return {
    ...
    agents:
      projectAgentsFile && !projectAgentsFile.error
        ...
        : undefined,
    projectTools:
      projectToolsFile && !projectToolsFile.error
        ? toOptionalEntry(projectToolsFile.path, projectToolsFile.content)
        : undefined,
    ...
    globalProfile: globalProfilePath
      ? toOptionalEntry(globalProfilePath, globalProfileContent)
      : undefined,
    globalFocus: globalFocusPath ? toOptionalEntry(globalFocusPath, globalFocusContent) : undefined,
    projectProfile:
      projectProfileFile && !projectProfileFile.error
        ? toOptionalEntry(projectProfileFile.path, projectProfileFile.content)
        : undefined,
    projectFocus:
      projectFocusFile && !projectFocusFile.error
        ? toOptionalEntry(projectFocusFile.path, projectFocusFile.content)
        : undefined,
    globalDailyMemory,
    projectDailyMemory,
    version: cachedLayeredSnapshot.version,
  ...
  layeredMemoryWatchCleanup = ipc.on(IPC.FS_FILE_CHANGED, (...args: unknown[]) => {
    ...
    void loadLayeredMemorySnapshot(ipc, {
      ...
      scope:
        ...
        cachedLayeredSnapshot.globalProfile ||
        cachedLayeredSnapshot.globalFocus ||
        cachedLayeredSnapshot.projectTools ||
        cachedLayeredSnapshot.projectMemory ||
        cachedLayeredSnapshot.projectProfile ||
        cachedLayeredSnapshot.projectFocus ||
...
export async function loadLayeredMemorySnapshot(
  ...
  const primaryWatchPath = cachedLayerSshConnectionId
    ? cachedLayeredSnapshot.globalMemory?.path ||
      cachedLayeredSnapshot.globalProfile?.path ||
      cachedLayeredSnapshot.globalFocus?.path
    : cachedLayeredSnapshot.globalMemory?.path ||
      cachedLayeredSnapshot.globalProfile?.path ||
      cachedLayeredSnapshot.globalFocus?.path ||
      cachedLayeredSnapshot.agents?.path ||
      cachedLayeredSnapshot.projectTools?.path ||
      cachedLayeredSnapshot.projectProfile?.path ||
      cachedLayeredSnapshot.projectFocus?.path
